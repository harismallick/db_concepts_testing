...
def increment_counter(conn: psycopg2.extensions.connection, id: int) -> None:
    lock: Lock = Lock(conn, str(id))
    try:
        # Use this approach if you want to retry connection if lock is not acquired:
        is_lock: bool = lock.acquire(block=False)
        if not is_lock:
            raise Exception("Could not acquire lock.")
        
        # Use this approach if you want to block thread until lock is acquired:
        # The worker thread will wait indefinitely for the lock. 
